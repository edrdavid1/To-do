    ...
    pageTodos.forEach(todo => {
      const li = createTodoItem(
        ...
        async (_id) => {
          ...
          const newTotalPages = Math.ceil(todos.length / ITEMS_PER_PAGE) || 1;
        ...
        async (_id) => {
          ...
          if (todo) {
            ...
            console.log('Updating todo with ID:', _id, 'and completed status:', todo.completed);
            await updateTodo(_id, { completed: todo.completed });
            renderPage(currentPage);
          }
        },
        async (_id, newText) => {
          ...
          if (todo) {
            todo.title = newText;
            console.log('Updating todo with ID:', _id, 'and new title:', newText);
